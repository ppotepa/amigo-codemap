...
pub fn parse_operations(text: &str) -> Vec<OperationEntry> {
    let token_re = Regex::new(r"(?i)tokens:\s*(?:used\s*)?~?([0-9]+)|tokens(?: used)?:\s*~?([0-9]+)").unwrap();
    text.split("\n### ")
        .filter_map(|section| {
            ...
            let task = trimmed.lines().next().unwrap_or("unknown").trim_matches('#').trim().to_string();
...
pub fn print_operations_summary(root: &std::path::Path, limit: usize) -> Result<()> {
    ...
    for entry in &entries {
        for key in ["rg", "npm build", "cargo test", "registry", "commands/mod.rs", "workspaceRuntimeServices"] {
    ...
    print_next(&["prioritize high repeated patterns", "add fixtures for expensive workflows"]);
    Ok(())
}
...
pub fn print_commit_summary(map: &CodeMap, limit: usize) {
    ...
    for change in &map.git.changed {
        *groups.entry(group_path(&change.path, Some("feature"))).or_default() += 1;
    }
    println!("affected:");
    ...
    print_next(&["run suggested checks", "write final response from affected areas"]);
}
